...
def predict():
    ...
    engine = sqlalchemy.create_engine("mysql+pymysql://{user}:{pw}@{host}/{db}".format(
        user=user, pw=password, host=endpoint, db='users_web_db'))
...
def update():
    ...
    if cond:
        ...
        weekly = new_data_table.set_index('date').asfreq(
            'D').groupby(pd.Grouper(freq='W')).sum()
        ...
        predictions_table = predictions_table[predictions_table.date >
                                              next_predicted_date]
        if mape >= 0.2:
            do_train(users_table)
        # push de tablas a AWS
        with engine.connect() as con:
            new_data_table.to_sql(name='new_data', con=con,
                                  if_exists='replace', index=False)
            users_table.to_sql(name='users_web', con=con,
                               if_exists='replace', index=False)
            predictions_table.to_sql(
                name='predict_users', con=con, if_exists='replace', index=False)
        con.close()
...
def reset():
    ...
    with engine.connect() as con:
        nuevos_datos.to_sql(name='new_data', con=con,
                            if_exists='replace', index=False)
        datos.to_sql(name='users_web', con=con,
                     if_exists='replace', index=False)
        con.close()
    ...
    # mandamos el dataframe a la nube
    with engine.connect() as con:
        pred_table.to_sql(name='predict_users', con=con,
                          if_exists='replace', index=False)
        con.close()
